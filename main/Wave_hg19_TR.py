...
def write_vcf(args, df, seqname, start_base, end_base):
    ...
    # with open(args.save + 'BV_' + str(args.rep) + '_seq_' + str(seqname) + ".vcf", 'w') as f:
        ...
        f.write('##source=Wave_hg19_TR.py\n')
...
def process_sv_data(empirical_data: List[float],
    ...
    try:
        if mode == 'probability':
            ...
            empirical_ints = [int(round(x)) for x in empirical_data]
            total_sv = int(sum(empirical_ints))
            print(f'Total {data_type.upper()}: {total_sv}')
...
def gen_consensus(xun, starts_seg, ends_seg, tem_seq_post, del_SV_per_segment, ins_SV_per_segment, del_snv_per_segment,ins_snv_per_segment,snp_per_segment, pai_pro_tem_, args, condition_dist_sv_del, len_SV_del,condition_dist_sv_ins,len_SV_ins,ins_selection,base_list,substitution_matrix,mis_selection,n_times,tmp_dir, TR_set, p_del_TR, p_ins_TR): 
    # 设置进程特定的种子
    ...
    if args.snp is None:
        ...
        num_snp = min(True_snp_number, len_unblock_region)
    
    max_length_numpy = True_del_number + True_ins_number + del_snv_number + ins_snv_number + num_snp
        ...
        for del_index in range(True_del_number):
            ...
            while (r_s+l_s>=len_seg_refine2) or (r_s+l_s-1 not in unblock_region_seg) or (unblock_region_seg.index(r_s+l_s-1)-unblock_region_seg.index(r_s)<l_s-1):
                ...
                if circular_count_del > args.times:
                    circular_count_del_break = 1
                    print("Warning: No."+str(del_index)+ "  long deletion sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase times or reference length.')
                    break
                else:
                    circular_count_del_break = 0
            if not circular_count_del_break:
    ...
    if not circular_count_micro_del_break:
        ...
        for m_del in range(0,del_snv_number):
            ...
            while (r_s+l_s>len_seg_refine) or (r_s+l_s-1 not in unblock_region_seg) or (unblock_region_seg.index(r_s+l_s-1)-unblock_region_seg.index(r_s)<l_s-1):
                ...
                if circular_count_del> args.times:
                    circular_count_del_break = 1
                    print("Warning: No."+str(m_del)+ "  small deletion sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase times or reference length.')
                    break
                else:
                    circular_count_del_break = 0
...
def main():
    ...
    for s in range(0, args.sv_trans):
        ...
        while (r_s+l_s > len_seg_refine) or (r_s+l_s-1 not in unblock_region_sv) or ((unblock_region_sv.index(r_s+l_s-1)-unblock_region_sv.index(r_s)<l_s-1)):
            ...
            if circular_count_trans>args.times:
                ...
                print('Warning: No.'+str(s)+ " translocation sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase times or reference length.')
                break
            else:
                circular_count_trans_break = 0
            #l_s = np.random.poisson(lamnuma_del, 1)[0]
        ...
        if not circular_count_trans_break:
            ...
            if p_rand_trans_b>ratio_b_trans:### original place is also deleted (cut A)
                ...
                while (r_s2+l_s2>len_seg_refine) or (r_s2+l_s2-1 not in unblock_region_sv) or (unblock_region_sv.index(r_s2+l_s2-1)-unblock_region_sv.index(r_s2)<l_s2-1):
                    ...
                    if circular_count_trans>args.times:
                        circular_count_trans_break = 1
                        print('Warning: No.'+str(s)+ " balanced translocation sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase times or reference length.')
                        break
                    else:
                        circular_count_trans_break = 0
    ...
    for inver_num in range (0, args.sv_inver):
        ...
        while (r_s+l_s>len_seg_refine) or (r_s+l_s-1 not in unblock_region_sv) or (unblock_region_sv.index(r_s+l_s-1)-unblock_region_sv.index(r_s)<l_s-1):
            ...
            if circular_count_inver>args.times:
                ...
                print("Warning: No."+str(inver_num)+ " inversion sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase times or reference length.')
                break
            else:
                circular_count_inver_break = 0
    ...
    for remain_index2 in dup_site_collection:##others### questions
        ...
        while len(set(range(remain_index2-tem_copied_base+1,remain_index2+1))&set(unblock_region_sv)) < tem_copied_base:
            ...
            if circular_count_dup>args.times:
                ...
                print("Warning: duplication sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase times or reference length.')
                break
            else:
                circular_count_dup_break = 0
    ...
    # 读取BED文件
    df_hg002 = pd.read_csv(main_url_empirical+'SV_Tier1_tel_dist_update.bed', sep='\t', header=None, names=['chr', 'pos', 'SV type','chr length','TR Group'])
    # print(df_hg002.head(5))
    # print(chr_name)
    #df_hg002 = pd.read_csv('/lustre/scratch/Stat/b158897/data/test_data/TGS/hg002/SV_Tier1_tel_dist_update.bed', sep='\t', header=None, names=['chr', 'pos', 'SV type','chr length','TR Group'])
    df_chr = df_hg002[df_hg002['chr'] == str(chr_name)]
    #chr_length = df_chr['chr length'].iloc[0]

    del df_hg002

    # bins_length = 5e5  # 每个Bin的长度
    # # 计算整除部分和余数部分
    # devided_length = int(chr_length // bins_length)
    # remainder_length = chr_length - devided_length * bins_length

    # # 创建新的bins数组，最后一个bin的长度大于5e5
    # bins = np.arange(0, chr_length - remainder_length, bins_length).tolist()
    # bins.append(chr_length)
    ...
    #     result = pool.apply_async(gen_consensus, args=(xun, chr_id, ll_c, starts_seg, ends_seg, tem_seq_post, del_SV_per_segment, ins_SV_per_segment, del_snv_per_segment, ins_snv_per_segment, snp_per_segment, pai_pro_tem_, args, condition_dist_sv_del, len_SV_del, condition_dist_sv_ins, len_SV_ins, ins_selection,base_list,substitution_matrix,mis_selection,n_times, TR_set, p_del_TR, p_ins_TR,tmp_dir), callback=lambda _: monitor_memory(90, max_mem_usage, max_mem_usage_gb,tmp_dir), error_callback=error_handler)
    #     results.append((xun, result))
    ...
    with ProcessPoolExecutor(max_workers=4) as executor:
        futures = {
            executor.submit(
                ...
                mis_selection, n_times, tmp_dir,  TR_set, p_del_TR, p_ins_TR
    ...
    del_data = del_empirical  # 单样本DEL计数列表
    ins_data = ins_empirical  # 单样本INS计数列表
