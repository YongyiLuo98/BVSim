...
# Base URL for GRCh38 resources
BASE_URL="https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/release/references/GRCh38/resources"

# Download the hg38 simpleRepeat files if not already present
if [ ! -f "$OUT_DIR/hg38.simpleRepeat.bed.gz" ]; then
    wget -P "$OUT_DIR" "$BASE_URL/hg38.simpleRepeat.bed.gz"
fi

if [ ! -f "$OUT_DIR/hg38.simpleRepeat.bed.gz.tbi" ]; then
    wget -P "$OUT_DIR" "$BASE_URL/hg38.simpleRepeat.bed.gz.tbi"
fi
...
for chr in {1..21}; do
    ...
    zcat "$OUT_DIR/hg38.simpleRepeat.bed.gz" | awk -v chr="chr${chr}" 'BEGIN{OFS="\t"} $1 == chr {print $1, $2, $3}' > "$OUT_DIR/windows_TR_chr${chr}.bed"
