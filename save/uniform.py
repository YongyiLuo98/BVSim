...
def parse_args():
    ...
    # parser.add_argument('-ref', '--main_url_input_ref', type=str, help='Input reference URL', default='default_ref')
    # parser.add_argument('-save', '--main_url_save', type=str, help='Save URL', default=os.path.expanduser('~'))
    # parser.add_argument('-seed', '--seed_con', type=int, help='Seed for random number generator', default=999)
    # parser.add_argument('-times', '--n_times', type=int, help='Number of times', default=10)
    # parser.add_argument('-rep', '--rep_id', type=int, help='Replication ID', default=1)
    # parser.add_argument('-sv_trans', '--num_trans_sv', type=int, help='Number of trans SV', default=2)
    # parser.add_argument('-sv_inver', '--num_inver_sv', type=int, help='Number of inversion SV', default=3)
    # parser.add_argument('-dup', '--num_dup_sv', type=int, help='True duplication number', default=4)
    # parser.add_argument('-sv_del', '--num_del_sv', type=int, help='Number of deletion SV', default=8)
    # parser.add_argument('-sv_ins', '--num_ins_sv', type=int, help='True insertion number', default=8)
    # parser.add_argument('-snp', '--num_snp', type=float, help='SNV number or probability', default=None)
    # parser.add_argument('-snv_del', '--num_del_snv', type=float, help='SNV deletion number or probability', default=None)
    # parser.add_argument('-snv_ins', '--num_ins_snv', type=float, help='SNV insertion number or probability', default=None)
    parser.add_argument('-ref', type=str, help='Input reference URL', default='default_ref')
    parser.add_argument('-save', type=str, help='Save URL', default=os.path.expanduser('~'))
    ...
    parser.add_argument('-dup', type=int, help='True duplication number', default=4)
...
def main():
    ...
    print('Inversion:'+str(args.num_inver_sv))
    print('DUP:'+str(args.num_dup_sv))
    print('DEL:'+str(args.num_del_sv))
    print('INS:'+str(args.num_ins_sv))
    ...
    if args.num_del_snv is None:
        del_snv_number = int(l_s_vec_ini[0])
    elif 0 <= args.num_del_snv < 1:
        del_snv_number = int(len_unblock_region * args.num_del_snv)
    else:
        del_snv_number = min(int(args.num_del_snv), len_unblock_region)

    if args.num_ins_snv is None:
        ins_snv_number = (np.random.binomial(len_left_region,diff_ins_prob_ins_real,1))[0]
    elif 0 <= args.num_ins_snv < 1:
        ins_snv_number = int(len_left_region * args.num_ins_snv)
    else:
        ins_snv_number = min(int(args.num_ins_snv), len_left_region)

    if args.num_snp is None:
        num_snp=int(l_s_vec_ini[1])
    elif 0 <= args.num_snp < 1:
        num_snp = int(len_unblock_region * args.num_snp)
    else:
        num_snp = min(int(args.num_snp), len_unblock_region)

    max_length_numpy = args.sv_trans + args.num_inver_sv + args.num_dup_sv + args.num_del_sv + args.num_ins_sv + del_snv_number + ins_snv_number + num_snp
    ...
    for s_trans in range(0, args.sv_trans):
        ...
        while (r_s+l_s > len_seg_refine) or (r_s+l_s-1 not in unblock_region_sv) or ((unblock_region_sv.index(r_s+l_s-1)-unblock_region_sv.index(r_s)<l_s-1)):
            ...
            if circular_count_trans>args.n_times:
                circular_count_trans_break = 1
                print('Warning: No.'+str(s_trans)+ " translocation sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase n_times or reference length.')
                break
            else:
                circular_count_trans_break = 0
            #l_s = np.random.poisson(lamnuma_del, 1)[0]
        ...
        if not circular_count_trans_break:
            ...
            if p_rand_trans_b>ratio_b_trans:### original place is also deleted (cut A)
                ...
                while (r_s2+l_s2>len_seg_refine) or (r_s2+l_s2-1 not in unblock_region_sv) or (unblock_region_sv.index(r_s2+l_s2-1)-unblock_region_sv.index(r_s2)<l_s2-1):
                    ...
                    if circular_count_trans>args.n_times:
                        circular_count_trans_break = 1
                        print('Warning: No.'+str(s_trans)+ " balanced translocation sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase n_times or reference length.')
                        break
                    else:
                        circular_count_trans_break = 0
    ...
    #! Inversion
    for inver_num in range (0, args.num_inver_sv):
        ...
        while (r_s+l_s>len_seg_refine) or (r_s+l_s-1 not in unblock_region_sv) or (unblock_region_sv.index(r_s+l_s-1)-unblock_region_sv.index(r_s)<l_s-1):
            ...
            if circular_count_inver>args.n_times:
                circular_count_inver_break = 1
                print("Warning: No."+str(inver_num)+ " inversion sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase n_times or reference length.')
                break
            else:
                circular_count_inver_break = 0
    ...
    #all_selected_dup_SV= sample(left_del_region_sv,args.num_dup_sv)
    all_selected_dup_SV= sample(unblock_region_sv,args.num_dup_sv)
    # 打印所有被选择的位点的信息
    ...
    for remain_index2 in dup_site_collection:##others### questions
        ...
        while len(set(range(remain_index2-tem_copied_base+1,remain_index2+1))&set(unblock_region_sv)) < tem_copied_base:
            ...
            if circular_count_dup> args.n_times:
                circular_count_dup_break = 1
                print("Warning: duplication sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase n_times or reference length.')
                break
            else:
                circular_count_dup_break = 0
    ...
    # args.num_del_sv = list(list_prob_sv).index(1)+1

    # args.num_del_sv=3

    for del_num in range(0, args.num_del_sv):
        ...
        while (r_s+l_s>len_seg_refine) or (r_s+l_s-1 not in unblock_region_sv) or (unblock_region_sv.index(r_s+l_s-1)-unblock_region_sv.index(r_s)<l_s-1):
            ...
            if circular_count_del>args.n_times:
                circular_count_del_break = 1
                print("Warning: No."+str(del_num)+ "  long deletion sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase n_times or reference length.')
                break
            else:
                circular_count_del_break = 0
    ...
    if not circular_count_ins_break:
        # print('SV INS:'+str(args.num_ins_sv))
        ...
        if len(left_del_region_sv) < args.num_ins_sv:
            ins_SV_pos = left_del_region_sv
            print('Warning: no enough positions for '+str(args.num_ins_sv)+' long insertions. Only '+str(len(ins_SV_pos))+' long insertions will be generated.')
        else:
            # 否则，随机选择位点
            ins_SV_pos = random.sample(left_del_region_sv, args.num_ins_sv)
        # 将被选择的位点添加到列表中
    ...
    if not circular_count_micro_del_break:
        ...
        if args.num_del_snv is None:
            ...
            if args.num_del_snv < 1:
                input_del = int(len_unblock_region * args.num_del_snv)
            else:
                input_del = int(args.num_del_snv)
        ...
        for m_del in range(0,del_snv_number):
            ...
            while (r_s+l_s>len_seg_refine) or (r_s+l_s-1 not in unblock_region_sv) or (unblock_region_sv.index(r_s+l_s-1)-unblock_region_sv.index(r_s)<l_s-1):
                ...
                if circular_count_del>args.n_times:
                    circular_count_del_break = 1
                    print("Warning: No."+str(m_del)+ "  micro deletion sampling times exceeds "+str(n_times)+' times. Try: reduce number of variations, increase n_times or reference length.')
                    break
                else:
                    circular_count_del_break = 0
            # if sample a del start and length that is not overlapped with translocation(s)
    ...
    if not circular_count_snv_break:
        ...
        if args.num_snp is None:
            ...
            num_snp = min(int(args.num_snp), len(unblock_region_sv))
        
        #num_snp=test_number
        ### substitution
        for number_mis in range(0,num_snp):
            #pos of substitution
    ...
    def write_template_fasta_con(args, seqname, consensus_, con_id):
        ...
        with open(args.save + 'GW_' + str(args.rep) + "_con" + str(con_id) + "_chr"+str(seqname) +"_ref.fasta", "w") as output_handle:
    ...
    def write_vcf(args, df, seqname, start_base, end_base,con_id):
        ...
        with open(args.save +'GW_' + str(args.rep) +"_con" + str(con_id) +  "_" +'chr'+str(seqname) +"_INDEL.vcf", 'w') as f:
    ...
    if args.write:
        ...
        SV_table_merged.to_csv(args.save +'GW_' + str(args.rep) +'_con'+str(ll_c)+'_chr'+str(seqname)+ '_SVtable_full.csv', header=True, index=False)
    else:
        SV_table_merged.to_csv(args.save +'GW_' + str(args.rep) +'_con'+str(ll_c)+'_chr'+str(seqname)+ '_SVtable.csv', header=True, index=False)
        # Save the dictionary as a .npy file
        np.save(args.save+'GW_'+str(args.rep)+'con'+str(ll_c)+'_chr'+str(seqname)+'_tem_ins_dic.npy', tem_ins_dic)
